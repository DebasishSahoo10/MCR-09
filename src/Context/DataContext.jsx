...
const handleActions = (state,action) => {
    switch (action.type) {
        case "ADD_TO_WL" : return {...state, watchLater : [...state.watchLater, action.payload]}
        case "REMOVE_FROM_WL" : return {...state, watchLater : state.watchLater.filter(vidID => vidID!==action.payload)}
        default : return state;
    }
}

export const DataProvider = ({children}) => {
    const [state, dispatch] = useReducer(handleActions, {videos : videos, categories : categories, playlists : [], watchLater : []})
    return (
